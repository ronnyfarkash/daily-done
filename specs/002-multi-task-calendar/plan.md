...
- Existing 001 Spec Kit artifacts: use only as historical baseline, legacy V1
  storage reference, and migration-test input. The approved 002 feature
  supersedes the prior one-task/non-multi-task scope for implementation.
...
- Do not use UTC to decide whether a task is planned, due today, missed, or
  completed.
- Re-evaluate the browser-local current date before status-sensitive actions
  such as completion and when the page regains focus, so planned/due/missed
  status does not rely on stale load-time state if the local day changes while
  the app is open.
...
- Re-evaluating task status when the supplied current local date changes.
