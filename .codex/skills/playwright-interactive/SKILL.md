...
description: "Persistent browser and Electron interaction through `js_repl`, with `playwright-repl` as a terminal fallback, for fast iterative UI debugging."
---
...
If `js_repl` is not exposed by the current Codex client, use the terminal
`playwright-repl` fallback in this workspace. It provides a persistent browser
session through a TTY process and supports keyword commands plus JavaScript
against the live `page` and `context` objects.
...
## Terminal Fallback: playwright-repl

Use this path when `js_repl` is enabled in config but not present in the active
tool list, which can happen in some VS Code-hosted Codex sessions.

Start the app's dev server in a persistent TTY:
...
```bash
npx --yes playwright-repl --session <project-name> --headed --include-snapshot
```
...
For complex QA, keep logic in a temporary module and call it from the REPL with
the live browser handle:

```javascript
const qaMod = await import("file:///tmp/project-repl-qa.mjs");
const result = await qaMod.runQa(context.browser());
...
- Use `context.browser().newContext(...)` for separate mobile and desktop
  contexts.
- Some terminal hosts parse dot meta-commands such as `.help` as JavaScript; if
  that happens, use keyword commands (`open`, `snapshot`) and normal JavaScript.
- Clean up by closing extra contexts in the QA module. Exit the REPL process
  when the browser QA is complete.
